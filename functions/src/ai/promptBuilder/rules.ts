/**
 * Rules and instructions for system prompt
 */

export function getRules(currentState: any, todayFormatted: string): string {
  const twoYearsAgo = new Date(new Date().getFullYear() - 2, new Date().getMonth(), new Date().getDate()).toISOString().split('T')[0];
  
  return `Context Resolution Rules (Handle Erratic Conversations):
...
7. For dates, use ISO format (YYYY-MM-DD). When user says relative terms:
   - "two years ago" or "for the last two years" → Calculate from current date (${todayFormatted}), so two years ago = ${twoYearsAgo}
   - "for the past X years/months" → Start date = current date minus X years/months
   - "since [date]" → Use the exact date they provide
   Example: If today is ${todayFormatted} and they say "living there for the last two years", set fromDate to approximately ${twoYearsAgo}
